...
public class RepairPointTracker : MonoBehaviour
{
    [Header("目标设置")]
    [Tooltip("跟踪主体（宇航员）")]
    public Transform astronaut; // 保持原命名，实际对应宇航员

    [Tooltip("需要跟踪的单个修理点")]
    public RepairPoint targetRepairPoint; // 改为关联RepairPoint脚本，用于判断修理状态
    ...
    [Tooltip("是否只显示未修理的点（修理完成后自动隐藏）")]
    public bool trackOnlyUnrepaired = true; // 新增：未修理点过滤开关
    ...
    {
        // 检查必要引用
        if (astronaut == null || targetRepairPoint == null || mainCamera == null ||
        ...
        // 新增：过滤逻辑——如果开启"只跟踪未修理"且修理点已完成，直接隐藏
        if (trackOnlyUnrepaired && targetRepairPoint.IsRepaired)
        {
            SetIndicatorActive(false);
            return;
        }
        ...
        // 计算航天器到空间站的距离
        float distance = Vector3.Distance(astronaut.position, targetRepairPoint.transform.position);
    ...
    {
        // 将空间站世界坐标转换为屏幕坐标
        Vector3 stationScreenPos = mainCamera.WorldToViewportPoint(targetRepairPoint.transform.position);
        ...
        // 计算箭头指向(指向空间站方向)
        Vector3 directionToStation = (targetRepairPoint.transform.position - astronaut.position).normalized;
        Vector3 directionInScreen = mainCamera.WorldToScreenPoint(astronaut.position + directionToStation) -
                                   mainCamera.WorldToScreenPoint(astronaut.position);
